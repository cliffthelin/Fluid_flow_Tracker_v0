import { getAllUroLogs, getAllHydroLogs } from "./db"
...
export async function createAutoBackup(): Promise<boolean> {
  try {
    ...
    if (uroLogs.length === 0 && hydroLogs.length === 0) {
    ...
    console.log(`Auto-backup created with ${uroLogs.length} uroLogs and ${hydroLogs.length} hydroLogs`)
    return true
  } catch (error) {
...
export async function restoreFromAutoBackup(): Promise<boolean> {
  try {
    ...
    const { bulkAddUroLogs, bulkAddHydroLogs, db } = await import("./db")
    ...
    if (uroCount === 0 && hydroCount === 0) {
      ...
      console.log(`Restored ${backup.uroLogs.length} uroLogs and ${backup.hydroLogs.length} hydroLogs from auto-backup`)
      return true
    } else {
