...
const DataManagement: React.FC<DataManagementProps> = ({
  ...
  const shareData = (monthKey?: string) => {
    ...
    // Add detailed logs
    const maxEntriesToShow = monthKey ? entriesToShare.length : Math.min(entriesToShare.length, 50)

    if (maxEntriesToShow > 0) {
      summary += "DETAILED FLOW LOGS\n"
      summary += "----------------\n\n"

      // Sort entries by date (newest first)
      const sortedEntries = [...entriesToShare]
      ...
      if (entriesToShare.length > maxEntriesToShow) {
        summary += `... and ${entriesToShare.length - maxEntriesToShow} more entries\n\n`
      }
    }

    // Add fluid intake logs
    const maxFluidEntriesToShow = monthKey ? fluidEntriesToShare.length : Math.min(fluidEntriesToShare.length, 20)

    if (maxFluidEntriesToShow > 0) {
      summary += "FLUID INTAKE LOGS\n"
      summary += "----------------\n\n"

      // Sort entries by date (newest first)
      const sortedFluidEntries = [...fluidEntriesToShare]
        .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
        .slice(0, maxFluidEntriesToShow)
      ...
      if (fluidEntriesToShare.length > maxFluidEntriesToShow) {
        summary += `... and ${fluidEntriesToShare.length - maxFluidEntriesToShow} more entries\n\n`
      }
    }

    // Add footer
    summary += "\nGenerated by Flow Tracker App\n"
